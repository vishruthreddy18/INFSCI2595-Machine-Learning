...
df_low_high <- readr::read_csv(path_low_high, col_names = TRUE, show_col_types = TRUE)
```
...
The 9th model has the lowest rmse score and the highest r-squared score. So this model is the best model on the training dataset based on these performance metrics.
...
No, for model 2 and 6, the coefficient estimates are not the same for x and I(x^2). Model 6 has higher uncertainty than model 2.

**Is the level or amount of uncertainty associated with the coefficients consistent across the three models?**  

The level of uncertainty is not consistent for the 3 models. For model 6 and model 9, there is more uncertainty with the coefficients of x and I(x^2) as compared to model 2.
...
input_viz <- tibble::tibble(
  x = seq(-2.342417, 2.644329, length.out = 201)
)
```
...
ggplot(data = df_viz_1, mapping = aes(x = x)) + geom_line(aes(y = pred_trend))
...
ggplot(data = df_viz_1, mapping = aes(x)) + geom_point(data = df_low_high, mapping = aes(y = y), color = 'red', shape = 0) + geom_line( mapping = aes(y = pred_trend), size = 1)
...
df_viz_all %>% ggplot(mapping = aes(x = x)) + geom_line(mapping = aes(y = pred_trend)) + facet_wrap(~poly_degree)
...
df_viz_all %>% ggplot(mapping = aes(x = x)) + geom_line(mapping = aes(y = pred_trend)) + facet_wrap(~poly_degree, scales = "free_y")
...
df_viz_all %>% ggplot(mapping = aes(x=x)) + geom_point(data = df_low_high, mapping = aes(y=y), shape = 0, color = 'red') +geom_line(mapping = aes(y=pred_trend)) + facet_wrap(~poly_degree, scales="free_y")
...
Model 9 is the best performing model. The model seems to have understood the training data well. The trends are consistent with the training dataset.
...
set.seed(202214)
id_train <- sample(1 : nrow(df_low_high), size = floor(0.8 * nrow(df_low_high)))
...
df_train %>% ggplot(mapping = aes(x = x, y = y)) + geom_point(mapping = aes(y = y), color = 'red', shape = 0) + geom_point(data = df_holdout, mapping = aes(y = y), color = 'blue', shape = 0) 
...
Model 2 has the lowest RMSE score, hence, is the best.
No, it's not the same model.
The holdout set ranks Model 9 as one of the worst models. The model has learnt the training data too well, that is, we can observe that the model has overfit the training data.
...
From the Coefficient vs value graph, we can see that the value of the coefficients are more or less the same for all the intercepts except for I(x^4), I(x^6) and I(x^8). All the variables seem insignificant as there confidence intervals pass through 0.
...
```
The coefficient of x is statistically insignificant for model2 trained on the whole dataset. The intercept values are more or less the same for model2 for both the cases. Also, in both cases the intercept and the coefficient of I(x^2) is significant.
...
The model will be trained 15 times as we are performing 5 fold cross validation and that too repeated 3 times.
...
The most commonly used metric for regression is RMSE as it tells us how far the observations are from the actual values i.e error.
...
Model 3 is the best according to the resampling results as the RMSE value is the lowest.
...
**Are the coefficient estimates similar for those coefficients present in at least 2 of the models visualized in the previous figure?**  
The values of the intercept and the coefficients of x for model 3 and 4 are more or less the same for all three models. The confidence interval for the coefficient fo I(x^2) is the highest and also its insignificant. For model 2, the intercept and coefficients are statistically significant.

**Is the level or amount of uncertainty associated with the coefficients consistent across the three models?**  
No, the level of uncertainty is not consistent across all 3 models.
...
### linear relationship (degree 1)
set.seed(2001)
mod_lowlow_1 <- train(y~x, data = df_low_low, method = "lm", metric = my_metric, trControl = my_ctrl)
...
### quadratic relationship (degree 2)
set.seed(2001)
mod_lowlow_2 <- train(y~x + I(x^2), data = df_low_low, method = "lm", metric = my_metric, trControl = my_ctrl)
...
### cubic relationship (degree 3)
set.seed(2001)
mod_lowlow_3 <- train(y~x + I(x^2) + I(x^3), data = df_low_low, method = "lm", metric = my_metric, trControl = my_ctrl)
...
### degree 4
set.seed(2001)
mod_lowlow_4 <- train(y~x + I(x^2) + I(x^3) + I(x^4), data = df_low_low, method = "lm", metric = my_metric, trControl = my_ctrl)
...
### degree 5
set.seed(2001)
mod_lowlow_5 <- train(y~x + I(x^2) + I(x^3) + I(x^4) + I(x^5), data = df_low_low, method = "lm", metric = my_metric, trControl = my_ctrl)
...
### degree 6
set.seed(2001)
mod_lowlow_6 <- train(y~x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6), data = df_low_low, method = "lm", metric = my_metric, trControl = my_ctrl)
...
### degree 7
set.seed(2001)
mod_lowlow_7 <- train(y~x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7), data = df_low_low, method = "lm", metric = my_metric, trControl = my_ctrl)
...
### degree 8
set.seed(2001)
mod_lowlow_8 <- train(y~x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8), data = df_low_low, method = "lm", metric = my_metric, trControl = my_ctrl)
...
### degree 9
set.seed(2001)
mod_lowlow_9 <- train(y~x + I(x^2) + I(x^3) + I(x^4) + I(x^5) + I(x^6) + I(x^7) + I(x^8) + I(x^9), data = df_low_low, method = "lm", metric = my_metric, trControl = my_ctrl)
...
dotplot(lowlow_results, metric = "RMSE")
...
Yes, it identified the same model as the best model. Yes, model 9 performed poorly in both the cases.
...
The scatterplot in figure 6(a) has a pattern whereas, the scatterplot in figure 1(a) has no definite pattern. We can witness a trend in figure 6(a) but the trend is not evident in figure 1(a).
...
model 5 is the best due to low RMSE scores.
...
The best models identified are different. Yes, the poorest model before was 9 but now it's 1.
...
Changing the sample size changed the best performing model. When the noise level was changed, the models remained the same.
