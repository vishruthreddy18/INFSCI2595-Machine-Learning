...
iris %>% 
  ggplot(mapping = aes(x = Sepal.Length)) +
  geom_histogram(color = "navyblue", size = 1.55, fill = "gold", bins = 6)
...
iris %>% 
  ggplot(mapping = aes(x = Sepal.Length)) +
  geom_histogram(color = "navyblue", alpha = 0.5, size = 1.55, fill = "gold", bins = 6)
...
1. Median of color 'J' is the highest among the other colors and has the least number of outliers
2. Every color except 'H' is positively skewed
3. Outliers order: E>D>F>H>G>I>J
...
diamonds  %>% 
  ggplot(mapping = aes(x = color, y = price)) +
  geom_boxplot(mapping = aes(color = carat > median(carat))) + 
  facet_grid(clarity ~ cut) +
...
diamonds  %>% filter(cut == "Ideal", clarity == "IF", carat > median(diamonds$carat)) %>%
 ggplot(mapping = aes(x = color, y = price)) +
  geom_boxplot(mapping = aes(color = carat > median(carat))) + 
  facet_grid(clarity ~ cut) +
  theme(legend.position = "top")
...
1. The skewness has drastically reduced for all the colors from figure 2b to 2f.
2. The colors F and G for the false case and I and J for the true case are negatively skewed in the figure 2f.
3. I and J in figure 2f have the highest level of agreement for the false case but the range is significantly high for I and J in figure 2b.
...
mod1 <- lm(formula = hwy ~ displ, data = mpg)
```
...
mod2 <- lm(formula = hwy ~ displ + drv, data = mpg)
```
