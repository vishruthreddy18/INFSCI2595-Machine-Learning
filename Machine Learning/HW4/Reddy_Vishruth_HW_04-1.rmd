...
The result is the same. The MLE based on counting is the sum of all the hits divided by the total number of event, which is the same as the mean of the vector x.
...
$$
\log(p(x_n|\mu)) = \log(Bernoulli(x_n|\mu))
$$
...
$$
\log(p(x_n|\mu)) = \log(\mu^{x_n}) + log(1-\mu)^{1-x_n}
$$

$$
\log \left(p\left(x_n \mid \mu\right)\right)=x_n \log (\mu)+\log (1-\mu)\left(1-x_n\right)
$$
...
log_bernoulli_pmf_1e <- function(xobs, prob)
{
   xprobs <- c()
   ...
   for (obs in xobs)
   {
     if (obs != 1)
     {
       xprobs <- append(xprobs, log(1-prob))
     }
     else
     {
       xprobs <- append(xprobs, log(prob))
     }
   }
   return (xprobs)
}

xprobs = log_bernoulli_pmf_1e(x, mu_mle)
xprobs
```
...
log_bernoulli_pmf_1f <- function(xn, prob)
{
  if (xn != 1)
  {
    return (log(1-prob))
...
for (itr in 1:5) 
{
  print(log_bernoulli_pmf_1f(x[itr], temp_mu))
}
...
log_bernoulli_pmf_1g <- function(xobs, prob)
{
   xprobs <- c()
   for (obs in xobs)
   {
...
xprobs = log_bernoulli_pmf_1g(x[1:5], 0.25)
xprobs
...
$$
\log (p(x|\mu)) = \sum_{n = 1} ^ {n = N}  \log (\mu ^ {x_n} . (1 - \mu) ^ {1 - x_n})
$$
...
log_bernoulli_pmf_1e <- function(xobs, prob)
{
  ...
  for (i in 1:n)
  {
      ...
      mu_xn = prob ^ xn
      mu_xn_ = (1 - prob) ^ (1 - xn)
      log_like = log_like + log(mu_xn * mu_xn_)
  }
...
mu_grid <- seq(0.025, 0.975, length.out = 251)
mu_grid
...
log_bernoulli_pmf_2e <- function(xobs, prob)
{
  log_like = 0
  xprobs <- c()
  n <- length(xobs)
...
for (mu in mu_grid)
{
  log_like = log_bernoulli_pmf_2e(x, mu)
  log_lik_xa <- append(log_lik_xa, log_like)
}
...
ggplot(data = xa_results) + 
  geom_line( mapping = aes(x=mu, y= log_lik)) + geom_vline(xintercept = mu_mle)
mu_mle
...
The trend decreases as the value of mu increases.
No, there is no change in the behavior.
...
tibble::tibble(
  ...
  mutate(log_lik = player_hits * log(mu) + (player_atbats - player_hits) * log(1 - mu) ) %>% ggplot(mapping = aes(x = mu, y = log_lik)) + geom_line(size = 1.1) + geom_vline(xintercept = mu_mle, size = 1, linetype = 'dashed', color = 'red')
...
In 3d, the graph peaks at 0.4.
In 2f, the trend is gradually decreasing.
...
tibble::tibble(
  ...
  mutate(log_lik = dbinom(x = player_hits, size = player_atbats, prob = mu, log = TRUE)) %>% ggplot(mapping = aes(x = mu, y = log_lik)) + geom_line(size = 1.1) + geom_line(size = 1.1) + geom_vline(xintercept = mu_mle, size = 1, linetype = 'dashed', color = 'red')
...
3e and 3c are the same but the y-axis values are different.
3e and 2f are not the same as the graph in 3e is peaking at 0.4 and the graph is gradually decreasing in 2f.
...
uniform_trials = a_uniform + b_uniform
inform_trials = a_inform + b_inform
...
tibble::tibble(
  ...
  geom_line() + geom_vline(xintercept = mean_uniform, size = 1, linetype = 'dashed', color = 'red')
...
tibble::tibble(
  ...
  ggplot(mapping = aes(x = mu, y=beta_pdf))+
  geom_line() + geom_vline(xintercept = mean_inform, size = 1, linetype = 'dashed', color = 'red')
...
In the first plot, all the intervals are equally likely.
In the second plot, the graph peaks at around 0.25 so the value will be around that range. It is unlikely for the values to go beyond 0.5.
...
$$ 
\log[P(\mu|m,N)]\propto\log(p(m|N,\mu))+log(p(\mu|a,b))
$$
$$
\log[P(\mu|m,N)]\propto(m\log (\mu)+(N-m)\log(\mu-1))+((a-1)\log(\mu)+(b-1)\log(\mu-1))
$$

$$
\log[P(\mu|m,N)]\propto((a+m-1)\log(\mu)+(b+N-m-1)\log(\mu-1))
$$

$$
Assume (a_{new}=a+m) and (b_{new}=b+N-m)
$$

$$
\log[P(\mu|m,N)]\propto((a_{new}-1)\log(\mu)+(b_{new}-1)\log(\mu-1))
$$

$$
\textbf{Similar to beta}
$$

$$
\log(p(\mu|m,N))\propto Beta(\mu|a_{new},b_{new})\propto Beta(\mu|a+m,b+(N-m))\\
$$
...
$$ 
\frac{\partial \log(p(\mu|m,N))}{\partial \mu} = \frac{\partial \log(\mu^{a_{new}-1} . (1-\mu)^{b_{new}-1})}{\partial \mu}\\
$$

$$
\frac{\partial (a_{new}-1)\log(\mu) + (b_{new}-1)(1-\mu))}{\partial \mu}
$$
...
$$
(a_{new} -1 )\frac{\partial log(\mu)} {\partial \mu} + (b_{new} -1 )\frac{\partial log(z)} {\partial \mu}
$$

$$
\textbf{Applying chain rule to solve the above equation,}
$$
...
$$
a_{new} - a_{new}{\mu_{MAP}} -1 + {\mu_{MAP}} - {\mu_{MAP}} b + {\mu_{MAP}} = 0
$$

$$
a_{new} - 1 -(a_{new}{\mu_{MAP}} + {\mu_{MAP}} b) + 2{\mu_{MAP}} = 0
$$
...
a_new_uniform_big = a_uniform + m_big
a_new_inform_big = a_inform + m_big
b_new_uniform_big = b_uniform + N_big - m_big
b_new_inform_big = b_inform + N_big - m_big

a_new = c(a_new_uniform_big, a_new_inform_big)
b_new = c(b_new_uniform_big, b_new_inform_big)

Combinations1 = c("uniform_big", "inform_big")

posterior_hp1 <- data.frame(Combinations1, a_new, b_new)
posterior_hp1
...
Mean_1 <- a_new_uniform_big / (a_new_uniform_big + b_new_uniform_big)
Mean_2 <- b_new_uniform_big / (a_new_uniform_big + b_new_uniform_big)
Mean_3 <- a_new_inform_big / (a_new_inform_big + b_new_inform_big)
Mean_4 <- b_new_inform_big / (a_new_inform_big + b_new_inform_big)
  
Mean <- c(Mean_1, Mean_2, Mean_3, Mean_4)

Mode_1 <- (a_new_uniform_big - 1) / (a_new_uniform_big + b_new_uniform_big - 2)
Mode_2 <- (b_new_uniform_big - 1) / (a_new_uniform_big + b_new_uniform_big - 2)
Mode_3 <- (a_new_inform_big - 1) / (a_new_inform_big + b_new_inform_big - 2)
Mode_4 <- (b_new_inform_big - 1) / (a_new_inform_big + b_new_inform_big - 2)

Mode <- c(Mode_1, Mode_2, Mode_3, Mode_4)

Quantile_0.05_1 <- qbeta(0.05, a_new_uniform_big, b_new_uniform_big)
Quantile_0.05_2 <- qbeta(0.05, a_new_inform_big, b_new_inform_big)
Quantile_0.05_big <- c(Quantile_0.05_1, Quantile_0.05_2)
  
Quantile_0.95_1 <- qbeta(0.95, a_new_uniform_big, b_new_uniform_big)
Quantile_0.95_2 <- qbeta(0.95, a_new_inform_big, b_new_inform_big)
Quantile_0.95_big <- c(Quantile_0.95_1, Quantile_0.95_2)

New <- c("a_new_uniform_big", "b_new_uniform_big", "a_new_inform_big", "b_new_inform_big")
summary_stats <- data.frame(New, Mean, Mode, Quantile_0.05_big, Quantile_0.95_big)
summary_stats
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu, a_new_uniform_big, b_new_uniform_big))%>%
  ggplot(mapping = aes(x = mu, y = beta_pdf))+
  geom_line()
```
...
tibble::tibble(
  ...
  mutate(beta_pdf = dbeta(mu, a_new_inform_big, b_new_inform_big)) %>%
  ggplot(mapping = aes(x = mu, y = beta_pdf)) +
  geom_line()
```
...
The graph is narrower in the second graph than the first with the increase in difference of a and b. The peak value has also decreased with the increase in differnece between a and b.
...
m_small <- season_hits
N_small <- season_atbats

a_new_uniform_small = a_uniform + m_small
b_new_uniform_small = b_uniform + N_small - m_small
  
a_new_inform_small = a_inform + m_small
b_new_inform_small = b_inform + N_small - m_small

a_new = c(a_new_uniform_small, a_new_inform_small)
b_new = c(b_new_uniform_small,b_new_inform_small)

Combinations = c("uniform_small","inform_small")

posterior_hp <- data.frame(Combinations, a_new, b_new)
posterior_hp
...
Mean_01<-a_new_uniform_small/(a_new_uniform_small+b_new_uniform_small)
Mean_02<-b_new_uniform_small/(a_new_uniform_small+b_new_uniform_small)
Mean_03<-a_new_inform_small/(a_new_inform_small+b_new_inform_small)
Mean_04<-b_new_inform_small/(a_new_inform_small+b_new_inform_small)
  
Mean<-c(Mean_01,Mean_02,Mean_03,Mean_04)

Mode_01<-(a_new_uniform_small-1)/(a_new_uniform_small+b_new_uniform_small-2)
Mode_02<-(b_new_uniform_small-1)/(a_new_uniform_small+b_new_uniform_small-2)
Mode_03<-(a_new_inform_small-1)/(a_new_inform_small+b_new_inform_small-2)
Mode_04<-(b_new_inform_small)/(a_new_inform_small+b_new_inform_small-2)

Mode<-c(Mode_01,Mode_02,Mode_03,Mode_04)

Quantile_0.05_01<-qbeta(0.05,a_new_uniform_small,b_new_uniform_small)
Quantile_0.05_02<-qbeta(0.05,a_new_inform_small,b_new_inform_small)
Quantile_0.05<-c(Quantile_0.05_01,Quantile_0.05_02)
  
Quantile_0.95_01<-qbeta(0.95,a_new_uniform_small,b_new_uniform_small)
Quantile_0.95_02<-qbeta(0.95,a_new_inform_small,b_new_inform_small)
Quantile_0.95<-c(Quantile_0.95_01,Quantile_0.95_02)

New1<-c("a_new_uniform_small","b_new_uniform_small","a_new_inform_small","b_new_inform_small")

summary_stats<-data.frame(New1,Mean,Mode,Quantile_0.05,Quantile_0.95)
summary_stats
...
tibble::tibble(
  ...
  mutate(beta_pdf =dbeta(mu,a_new_uniform_small,b_new_uniform_small))%>%
  ggplot(mapping=aes(x=mu,y=beta_pdf))+
  geom_line()
```
...
tibble::tibble(
  ...
  mutate(beta_pdf =dbeta(mu,a_new_inform_small,b_new_inform_small) )%>%ggplot(mapping=aes(x=mu,y=beta_pdf))+
  geom_line()
```
...
With the increase in sample size, the peak is taller and at a lower mu value.
